test = list(
  ...
  cases = list(
    ...
    ottr::TestCase$new(
      ...
      failure_message = "Almost ther! Ensure you have selected columns **DepDelay** and **DepDel15** and then filtered the data set to only include rows where the the value for **DepDel15** is NA.",
      code = {}
      suppressPackageStartupMessages( {
        library(testthat)
        library(ottr)
      ...
      ## Test ##
      test_that('data dimension correct', {
        expect_output(glimse(flight_depdel), "Rows: 2,761\nColumns: 2")
      })
    )
  )
)
